...
**Planned future trigger (not built yet, see [[ghl-signed-to-bt-notification]]):** eventually this skill should fire automatically off a notification from GHL's Signed stage, reading the design agreement's own custom fields as source material instead of a manual chat paste — that becomes the new #1 source ahead of everything in the list above once it exists. Until that's built, source-gathering stays manual as described above. This is a two-session project: the GHL-side trigger belongs to whichever session owns `~/nss-ghl`, never build or write that side from here.
