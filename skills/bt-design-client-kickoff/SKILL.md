...
**Planned future trigger (webhook not built yet, see [[ghl-signed-to-bt-notification]] for full detail):** eventually this skill should fire automatically off a GHL webhook that fires when the design agreement is signed, reading the design agreement's own custom fields as source material instead of a manual chat paste — that becomes the new #1 source ahead of everything in the list above once it exists. The GHL side (design agreement itself) is functional as of 2026-08-14; the field schema is confirmed (`project_scope`, `project_address`, `budget_range`, `design_retainer`, `retainer_percent`, `design_hourly_rate`, `agreement_date`, plus standard contact fields) — full IDs in the memory file. The webhook itself isn't built yet; GHL writes need Kyle at the terminal, so it's scheduled for whenever he's next there with the GHL session. Until it exists, source-gathering stays manual as described above. This is a two-session project: the GHL-side trigger belongs to whichever session owns `~/nss-ghl`, never build or write that side from here.

**The BT half of that trigger is already designed and sitting in this folder — two files:**
- `webhook-contract.md` — the spec the GHL session builds its outbound POST against: method, placeholder path (no receiver exists yet), exact JSON body, field-by-field mapping back to the GHL field IDs, shared-secret header, and retry/dedupe behavior. It's written to be implementable with no follow-up questions.
- `ghl_payload_adapter.py` — python3, stdlib only. Takes a payload matching that contract (`--file PATH` or stdin, `--out PATH` to also write a file, `--strict` to fail on missing required fields) and prints the Step 0 source-material block, formatted exactly the way a manual chat paste would be: client info, parsed BT-ready address, derived `ROM - LastName - Scope` job title, verbatim scope, retainer/budget numbers, plus the standing open questions from Step 3 that no GHL field can answer.

How they fit: GHL builds against the contract → the signed-agreement POST carries the fields → run the payload through the adapter → paste its output into a fresh session as Step 0. That paste is the entire remaining manual step, and it's the last mile on purpose: BT has no API, it's browser automation, so a live session with the Chrome extension has to do the clicking either way. The logical next increment (not built, don't assume it exists) is a small always-on local listener that receives the POST and runs the adapter automatically.
